...
import { convertSimple2RegExpPattern, isEmojiImprecise, startsWithIgnoreCase } from './strings';
...
function _matchesPrefix(ignoreCase: boolean, word: string, wordToMatchAgainst: string) {
...
export function matchesSubString(word: string, wordToMatchAgainst: string) {
...
const wordSeparator = new Set<number>()
'()[]{}<>`\'"-/;:,.?!'
  .split('')
  .forEach(s => wordSeparator.add(s.charCodeAt(0)))

function isWordSeparator(code: number) {
  return isWhitespace(code) || wordSeparator.has(code)
}

function charactersMatch(codeA: number, codeB: number) {
...
function nextAnchor(camelCaseCode: string, start: number) {
  for (let i = start; i < camelCaseCode.length; i++) {
    const c = camelCaseCode.charCodeAt(i)
    if (isUpper(c) || isNumber(c) || (i > 0 && !isAlphanumeric(camelCaseCode.charCodeAt(i - 1)))) {
      return i
    }
  }
  return camelCaseCode.length
}
...
export function matchesCamelCase(word: string, camelCaseWord: string): IMatch[] | null {
  ...
  const analysis = analysisCamelCaseWord(word)
...
function analysisCamelCaseWord(word: string): ICamelCaseAnalysis {
  ...
  return { upperPercent, lowerPercent, numericPercent, alphaPercent }
}

function isUpperCaseWord(analysis: ICamelCaseAnalysis) {
  const { upperPercent, lowerPercent } = analysis
  return lowerPercent === 0 && upperPercent > 0.6
}

function isCamelCaseWord(analysis: ICamelCaseAnalysis) {
  const { upperPercent, lowerPercent, numericPercent, alphaPercent } = analysis
  return lowerPercent > 0.2 && upperPercent < 0.8 && alphaPercent > 0.6 && numericPercent < 0.2
}

function isCamelCasePattern(word: string) {
...
function matchesWords(word: string, target: string, contiguous: boolean = false) {
...
function _matchesWords(word: string, target: string, i: number, j: number, contiguous: boolean): IMatch[] | null {
  if (i === word.length) {
    ...
    return null
  } else if (!charactersMatch(word.charCodeAt(i), target.charCodeAt(i))) {
    return null
  } else {
    let result: IMatch[] | null = null
...
function nextWord(word: string, start: number) {
...
const fuzzyRegExpCache = new LRUCache<string, RegExp>(10000)
...
export function fuzzyScoreGracefulAggressive(pattern: string, lowPattern: string, patternPos: number, word: string, lowWord: string, wordPos: number, options?: FuzzyScoreOptions) {
  return fuzzyScoreWithPermutations(pattern, lowPattern, patternPos, word, lowWord, wordPos, true, options)
}

export function fuzzyScoreGraceful(pattern: string, lowPattern: string, patternPos: number, word: string, lowWord: string, wordPos: number, options?: FuzzyScoreOptions) {
  return fuzzyScoreWithPermutations(pattern, lowPattern, patternPos, word, lowWord, wordPos, false, options)
}

function fuzzyScoreWithPermutations(pattern: string, lowPattern: string, patternPos: number, word: string, lowWord: string, wordPos: number, aggressive: boolean, options?: FuzzyScoreOptions) {
...
function nextTypoPermutation(pattern: string, patternPos: number) {
